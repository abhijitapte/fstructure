...
int TextIndex::Insert(const char *key, int recAddr){
    int i;
    int index = Find(key);
    if(Unique && index >= 0) return 0; // key present.
    if(NumKeys==MaxKeys) return 0; //index full.
    for(i=NumKeys-1; i>=0; i--){
        if(strcmp(key, Keys[i])>0) break; //insert @ i+1
        Keys[i+1] = Keys[i];
...
int TextIndex::Find(const char *key) const{
    for(int i=0; i<NumKeys; i++)
        ...
        else if(strcmp(key, Keys[i])>0) return -1;
    return -1;
}
